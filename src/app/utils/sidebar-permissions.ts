...
import { PermissionCheck, Resource, UserProfile, Action } from '@/src/types/rbac';
  ...
  return items
    .map((item) => {
      ...
      if (item.items && item.items.length > 0) {
        filteredSubItems = filterSidebarItems(item.items, user, canPerformAction);
...
function checkItemPermission(
  ...
  if (!item.permission && !item.requiredPermissions && !item.requiredAnyPermissions) {
    return true;
  }

  if (item.permission) {
    const hasPermission = canPerformAction(user, item.permission.resource, item.permission.action);
...
export function getActiveSidebarItem(items: SidebarItem[], pathname: string): SidebarItem | null {
