...
import { User, handleProtectedRoute } from '@/src/lib/apiRouteWrappers'
  ...
  async (req, { user, params, query }) => {
    try {
      ...
      if (!code) {
        return NextResponse.json(
          { error: 'Code parameter is required' },
          { status: 400, statusText: 'Code parameter is required' },
        )
      }
...
export const DELETE = handleProtectedRoute(
  async (req, { user, params, query }) => {
    try {
      const code = params?.code
      if (!code) {
